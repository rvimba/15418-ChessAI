...
vector<move_t> pawnMoves(Chessboard b, Color color, int8_t rank, int8_t file) {
    ...
    switch (color) {
        case White:
            ...
            checkPawnMoves(b, moves, White, rank, file, forward_1, forward_2, left, right, new_rank_1, new_rank_2);
            break;
        case Black:
            new_rank_1 = rank - 1;
            new_rank_2 = rank - 2;
            ...
            left = b.pieceAt(new_rank_1, file-1);
            right = b.pieceAt(new_rank_1, file+1);
            checkPawnMoves(b, moves, Black, rank, file, forward_1, forward_2, left, right, new_rank_1, new_rank_2);
            break;
        default:
            break;
    }
...
vector<move_t> knightMoves(Chessboard board, Color color, int8_t rank, int8_t file) {
    vector<move_t> moves;
...
vector<move_t> queenMoves(Chessboard b, Color color, int8_t rank, int8_t file) {
    vector<move_t> queen_moves;
    
    vector<move_t> rook_moves = rookMoves(b, color, rank, file);
    for (vector<move_t>::iterator it = rook_moves.begin(); it != rook_moves.end(); it++) {
                    queen_moves.push_back(*it);
    }

    vector<move_t> bishop_moves = bishopMoves(b, color, rank, file);
    for (vector<move_t>::iterator it = bishop_moves.begin(); it != bishop_moves.end(); it++) {
                    queen_moves.push_back(*it);
    }
